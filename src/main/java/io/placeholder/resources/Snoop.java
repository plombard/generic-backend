...
public class Snoop {
  ...
  public Response snoop(@Context ServletContext servletContext) {
    ...
    map.put("jvm.started", LocalDateTime.ofInstant(startInstant, ZoneId
        .systemDefault()).format(DateTimeFormatter.ISO_DATE_TIME));
    //map.put("jvm.properties", runtimeBean.getSystemProperties());
    MemoryMXBean memoryBean = ManagementFactory.getMemoryMXBean();
    map.put("heap.used", memoryBean.getHeapMemoryUsage().getUsed() / (1024 *
        1024));
    map.put("heap.max", memoryBean.getHeapMemoryUsage().getMax() / (1024 *
        1024));
    map.put("nonheap.used", memoryBean.getNonHeapMemoryUsage().getUsed() /
        (1024 * 1024));
    map.put("nonheap.max", memoryBean.getNonHeapMemoryUsage().getMax() /
        (1024 * 1024));
